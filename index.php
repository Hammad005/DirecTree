<!DOCTYPE html>
<html lang="en">

<head>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="author" content="Colorlib">
    <meta name="description" content="#">
    <meta name="keywords" content="#">
    <!-- Page Title -->
    <title>DirecTree</title>
    <link rel="apple-touch-icon" sizes="180x180" href="images/logo/favicon/apple-touch-icon.png">
    <link rel="icon" type="image/png" sizes="32x32" href="images/logo/favicon/favicon-32x32.png">
    <link rel="icon" type="image/png" sizes="16x16" href="images/logo/favicon/favicon-16x16.png">
    <link rel="manifest" href="images/logo/favicon/site.webmanifest">
    <link rel="mask-icon" href="images/logo/favicon/safari-pinned-tab.svg" color="#5bbad5">
    <meta name="msapplication-TileColor" content="#da532c">
    <meta name="theme-color" content="#ffffff">
    <!-- Bootstrap CSS -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet"
        integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">
    <!-- <link rel="stylesheet" href="css/bootstrap.min.css"> -->
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/aos@2.3.4/dist/aos.css">
    <!-- Google Fonts -->
    <link href="https://fonts.googleapis.com/css?family=Roboto:300,400,400i,500,700,900" rel="stylesheet">
    <!-- Simple line Icon -->
    <link rel="stylesheet" href="css/simple-line-icons.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.6.0/css/all.min.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/ionicons/4.5.6/css/ionicons.min.css">
    <!-- Themify Icon -->
    <link rel="stylesheet" href="css/themify-icons.css">
    <!-- Hover Effects -->
    <link rel="stylesheet" href="css/set1.css">
    <!-- Main CSS -->
    <link rel="stylesheet" href="css/style.css">
</head>

<body data-aos-easing="ease" data-aos-duration="400" data-aos-delay="0">
    <?php
    include 'partials/_nav.php'
    ?>
    <!-- SLIDER -->
    <section id="slider" class="d-flex align-items-center">
        <div class="overlay"></div>
        <div class="container-fluid">
            <div class="row no-gutters slider-text align-items-center justify-content-center">
                <div class="col-lg-8 text-center pb-5">
                    <div class="row justify-content-center">
                        <div class="col-lg-9">
                            <h1>Discover Great Places In Karachi</h1>
                            <p>Empowering Your Journey Through Every Category</p>
                        </div>
                    </div>
                    <form method="GET" action="searchResults.php" class="search-property-1 mt-md-5">
                        <div class="row g-0 align-items-center">
                            <div class="col-md-8">
                                <div class="form-group p-3">
                                    <div class="form-field">
                                        <div class="icon">
                                            <i class="fa-solid fa-magnifying-glass"></i>
                                        </div>
                                        <input type="text" name="search" class="form-control" placeholder="What are you looking for?"
                                            required>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group ">
                                    <button type="submit" class="btn btn-primary w-100 py-3">
                                        <i class="ion-ios-search"></i>
                                        Search
                                    </button>
                                </div>
                            </div>
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </section>
    <!-- SLIDER END -->
    <!--Categories Section -->
    <section id="main-block">
        <div class="container ">
            <div class="row g-0">
                <div class="col-md-12 services-wrap">
                    <div class="row g-3 justify-content-center">
                        <div class="col-md-4 col-lg-2 category-responsive text-center d-flex align-items-stretch"
                            data-aos="fade-up" data-aos-delay="100" data-aos-duration="1000">
                            <a href="food_drink.php" class="category-wrap">
                                <div class="category-block">
                                    <div class="category-img">
                                        <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                                            xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 503.607 503.607"
                                            xml:space="preserve">
                                            <g transform="translate(1 1)">
                                                <g>
                                                    <path
                                                        d="M485.82,82.92H376.705V46.109l54.557-31.176c4.197-2.518,5.875-7.554,3.357-11.751s-7.554-5.036-11.751-3.357 l-58.754,33.574c-0.418,0.251-0.794,0.533-1.147,0.831c-1.922,1.465-3.05,3.81-3.05,6.723V82.92H250.803 c-9.233,0-16.787,7.554-16.787,16.787v33.574c0,6.308,3.529,11.829,8.709,14.696l1.666,33.32 c-20.724-3.946-43.922-6.048-69.129-6.048C69.505,175.248-1,212.179-1,267.576v8.393c0,9.233,7.554,16.787,16.787,16.787h8.393 v18.997c-12.177,3.394-21.682,12.859-24.341,25.489c-1.679,9.233,0.839,18.466,6.715,26.02c4.225,5.432,10.622,9.55,17.626,11.754 v9.251C10.294,384.717-1,396.277-1,410.264v15.948c0,13.987,11.294,25.547,25.18,25.998v25.202c0,14.269,10.911,25.18,25.18,25.18 h251.803h151.921c13.43,0,24.341-10.911,25.18-24.341l15.649-330.285c5.17-2.87,8.692-8.384,8.692-14.686V99.707 C502.607,90.474,495.052,82.92,485.82,82.92z M250.803,99.707H485.82v33.574H250.803V99.707z M40.967,384.245v-8.446 c5.448-1.186,10.649-3.6,15.108-7.501c4.197-4.197,7.554-10.072,9.233-16.787h26.02c4.197,14.269,16.787,25.18,32.734,25.18 c15.948,0,29.377-10.911,32.734-25.18h36.092c4.197,14.269,16.787,25.18,32.734,25.18c15.948,0,29.377-10.911,32.734-25.18h26.859 c1.679,6.715,4.197,12.59,9.233,16.787c4.259,3.651,9.404,6.411,15.108,7.658v8.289H40.967z M338.692,316.947 c-3.615-2.959-7.811-5.23-12.348-6.564v-17.626h8.393c9.233,0,16.787-7.554,16.787-16.787v-8.393c0-1.129-0.037-2.25-0.092-3.365 c-0.005-0.089-0.009-0.178-0.014-0.267c-0.56-10.565-3.6-20.524-9.646-29.875c-0.369-0.588-0.756-1.171-1.146-1.752 c8.389-4.192,17.617-6.708,27.683-6.708c32.734,0,58.754,26.02,58.754,58.754s-26.02,58.754-58.754,58.754 c-5.875,0-10.911-0.839-16.787-2.518C350.966,331.104,345.945,322.728,338.692,316.947z M15.787,267.576 c0-44.485,65.469-75.541,159.475-75.541c76.159,0,133.582,20.385,152.643,51.778c4.549,7.698,6.832,15.73,6.832,23.762v8.393 h-16.787H32.574H15.787V267.576z M309.557,292.756v16.787H40.967v-16.787H309.557z M19.984,352.349 c-2.518-2.518-4.197-6.715-3.357-12.59c0.839-7.554,9.233-13.43,17.626-13.43h283.698c8.253,0,16.487,8.115,16.761,16.366 c-0.274,8.224-8.522,15.544-17.6,16.369c-0.979,0-1.909,0.236-2.765,0.639c-3.17-0.406-5.659-1.509-8.147-3.997 c-3.357-2.518-5.036-7.554-5.036-12.59c0-5.036-3.357-8.393-8.393-8.393h-41.967c-5.036,0-8.393,3.357-8.393,8.393 c0,9.233-7.554,16.787-16.787,16.787s-16.787-7.554-16.787-16.787c0-5.036-3.357-8.393-8.393-8.393h-50.361 c-5.036,0-8.393,3.357-8.393,8.393c0,9.233-7.554,16.787-16.787,16.787s-16.787-7.554-16.787-16.787 c0-5.036-3.357-8.393-8.393-8.393H57.754c-5.036,0-8.393,3.357-8.393,8.393c0,5.036-1.679,10.072-5.036,11.751 c-2.518,3.357-6.715,4.197-10.911,4.197C28.377,359.064,23.341,356.546,19.984,352.349z M15.787,427.051v-15.948 c0-10.911,9.233-10.072,9.233-10.072h7.554h285.377h8.393c5.036,0,9.233,4.197,9.233,9.233v15.948 c0,5.036-4.197,9.233-9.233,9.233h-8.393H32.574H25.02C19.984,435.445,15.787,431.248,15.787,427.051z M40.967,477.412v-25.18 h268.59v25.18c0,5.036-3.357,8.393-8.393,8.393H49.361C44.325,485.805,40.967,482.448,40.967,477.412z M461.479,477.412 c0,5.036-4.197,8.393-8.393,8.393H324.991c0.631-1.881,1.055-3.87,1.241-5.947c0.068-0.536,0.112-1.071,0.112-1.607v-0.839 v-24.341c11.206-0.659,20.338-7.532,23.744-17.363c1.277-2.949,2.079-6.151,2.276-9.496v-15.948 c0-14.269-11.751-26.02-26.02-26.02v-7.904c10.101-2.746,18.231-9.898,22.313-19.034c6.339,1.818,12.802,2.597,19.654,2.597 c41.967,0,75.541-33.574,76.38-76.38c0-41.967-33.574-75.541-75.541-75.541c-14.46,0-27.671,3.747-39.64,11.22 c-15.491-14.839-38.644-26.471-67.803-34.032l-1.672-35.103h217.39L461.479,477.412z" />
                                                </g>
                                            </g>
                                        </svg>
                                    </div>
                                    <h6>Food & Drink</h6>
                                    <p class="list">
                                        <?php
                                        $sql = "SELECT * FROM `listings` WHERE `listing_cat` = 'Food & Drink' AND `status` = 'approved'";
                                        $result = mysqli_query($conn, $sql);
                                        $count = mysqli_num_rows($result);
                                        $formattedCount = str_pad($count, 2, '0', STR_PAD_LEFT);

                                        if ($count === 1) {
                                            echo "<span>" . $formattedCount . "</span> Listing";
                                        } else {
                                            echo "<span>" . $formattedCount . "</span> Listings";
                                        }
                                        ?>
                                    </p>
                                </div>
                            </a>
                        </div>

                        <div class="col-md-4 col-lg-2 category-responsive text-center d-flex align-items-stretch"
                            data-aos="fade-up" data-aos-delay="100" data-aos-duration="1000">
                            <a href="hotels.php" class="category-wrap">
                                <div class="category-block">
                                    <div class="category-img">
                                        <svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg"
                                            xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" role="img"
                                            class="iconify iconify--emojione-monotone"
                                            preserveAspectRatio="xMidYMid meet">
                                            <path
                                                d="M63 60h-6V41.758c3.656 1.955 3.766 5.918 3.766 5.918c.697-5.52-1.842-7.643-3.823-8.463a5.888 5.888 0 0 0-.128-.666C60.835 35.27 64 36.633 64 36.633c-3.598-4.347-6.668-2.432-8.414-.492A5.983 5.983 0 0 0 51 34h-5v-8c0-3.308-2.691-6-6-6h-1v-6c0-1.077-.531-2.36-1.293-3.121l-1.586-1.587C35.36 8.532 34.076 8 33 8V6.558c2.666 1.533 3.332-1.534 6 0V1.443c-2.668-1.533-3.334 1.534-6 0V1c0-.55-.451-1-1-1c-.551 0-1 .45-1 1v7c-1.076 0-2.36.532-3.121 1.292l-1.586 1.587C25.531 11.64 25 12.923 25 14v6h-1c-3.309 0-6 2.692-6 6v8h-5a5.99 5.99 0 0 0-4.588 2.141c-1.744-1.94-4.815-3.856-8.412.492c0 0 3.164-1.363 7.186 1.916a5.811 5.811 0 0 0-.128.664c-1.981.82-4.521 2.943-3.823 8.463c0 0 .109-3.965 3.766-5.918V60H1c-.551 0-1 .451-1 1v2c0 .551.449 1 1 1h62c.549 0 1-.449 1-1v-2c0-.549-.451-1-1-1m-8.972-18.961c.167-.047.316-.102.493-.143c.336.063.646.145.947.234c.075.635.159 1.443.234 2.449h-1.727c.027-.821.044-1.673.053-2.54m-.061 2.889c.757.959 1.393.625 1.774.215c.057.836.104 1.781.139 2.834h-2.055c.056-.979.105-1.989.142-3.049M53 41.365a.13.13 0 0 0 .025-.01c-.025 1.818-.09 3.537-.177 5.15A.988.988 0 0 0 52 46h-3.639c.23-.936.75-2.023 1.832-3H52c.549 0 1-.449 1-1v-.635m-42 .002V42c0 .551.449 1 1 1h1.807c1.081.977 1.601 2.064 1.831 3H12a.989.989 0 0 0-.849.504c-.088-1.611-.151-3.33-.177-5.148l.026.011m-2.469-.236c.3-.09.61-.172.945-.234c.178.043.327.096.495.143c.01.867.026 1.719.051 2.541H8.296c.076-1.007.159-1.815.235-2.45m-.273 3.012c.383.41 1.019.744 1.775-.217c.035 1.061.084 2.07.142 3.051H8.117c.035-1.053.084-1.998.141-2.834m-.156 3.326c.434.424 1.187.828 2.098-.047c.063 1.037.135 2.018.213 2.949H8.07c-.002-1.035.008-2.008.032-2.902m-.03 3.367c.462.467 1.322.947 2.356-.266c.1 1.158.206 2.227.313 3.197H8.136a115.904 115.904 0 0 1-.064-2.931m.075 3.217c.43.451 1.414 1.174 2.647.182c.068.596.138 1.139.205 1.652V56c0 .078.027.148.045.223c.046.336.088.648.132.941H8.303a118.62 118.62 0 0 1-.156-3.111M8.512 60c-.078-.914-.143-1.795-.198-2.652c.382.443 1.497 1.441 2.91.139c.24 1.613.419 2.513.419 2.513H8.512M22 48.063L20.764 49l.469-1.525L20 46.529l1.525-.006L22 45l.475 1.523l1.525.006l-1.232.945l.468 1.526L22 48.063M24 63l1.333-1h13.334L40 63H24m4.236-14L27 48.063L25.764 49l.469-1.525L25 46.529l1.525-.006L27 45l.475 1.523l1.525.006l-1.232.945l.468 1.526M28 50v10h-3V50h3m5.236-1L32 48.063L30.764 49l.469-1.525L30 46.529l1.525-.006L32 45l.475 1.523l1.525.006l-1.232.945l.468 1.526M33 50v10h-2V50h2m5.236-1L37 48.063L35.764 49l.469-1.525L35 46.529l1.525-.006L37 45l.475 1.523l1.525.006l-1.232.945l.468 1.526M39 50v10h-3V50h3m1.764-1l.469-1.525L40 46.529l1.525-.006L42 45l.475 1.523l1.525.006l-1.232.945l.468 1.526L42 48.063L40.764 49M52 53h-4c-.551 0-1 .451-1 1v2c0 .551.449 1 1 1h3.833a62.876 62.876 0 0 1-.458 2.805l-.04.195H43V50h2v-6H19v6h2v10h-8.338l-.039-.195A61.86 61.86 0 0 1 12.166 57H16c.549 0 1-.449 1-1v-2c0-.549-.451-1-1-1h-4a.945.945 0 0 0-.324.066a122.284 122.284 0 0 1-.31-3.311A.98.98 0 0 0 12 50h4c.549 0 1-.449 1-1v-2c0-.414-.258-.771-.618-.922c.177-1.191.021-2.213-.352-3.084c.534-.017.97-.455.97-.994v-2c0-.549-.451-1-1-1h-4a.959.959 0 0 0-.409.094c-.11-.047-.222-.096-.327-.139c4.877-4.582 8.736-1.15 8.736-1.15A8.378 8.378 0 0 0 18.179 36H20V26c0-2.2 1.799-4 4-4h3v-8c0-.55.318-1.318.707-1.708l1.586-1.585C29.682 10.318 30.449 10 31 10h2c.549 0 1.318.318 1.707.708l1.586 1.585c.389.389.707 1.157.707 1.707v8h3c2.199 0 4 1.8 4 4v10h1.82A8.353 8.353 0 0 0 44 37.805s3.858-3.432 8.734 1.15c-.105.043-.217.092-.326.139A.95.95 0 0 0 52 39h-4c-.551 0-1 .451-1 1v2a1 1 0 0 0 .969.994c-.373.871-.528 1.893-.353 3.084c-.36.151-.616.508-.616.922v2c0 .551.449 1 1 1h4a.985.985 0 0 0 .633-.244a127.106 127.106 0 0 1-.309 3.311A.933.933 0 0 0 52 53m3.486 7h-3.131s.179-.9.418-2.512c1.414 1.301 2.528.303 2.911-.141c-.055.858-.12 1.739-.198 2.653m.209-2.836h-2.873l.129-.926c.021-.078.049-.154.049-.238v-.123c.067-.51.136-1.051.205-1.645c1.232.994 2.216.271 2.646-.18c-.037.993-.087 2.026-.156 3.112m.167-3.396h-2.606c.109-.971.216-2.039.314-3.199c1.033 1.213 1.894.736 2.355.27c-.006.923-.028 1.899-.063 2.929m-2.276-3.397c.078-.932.151-1.912.215-2.951c.909.875 1.663.473 2.096.051c.023.893.033 1.865.03 2.9h-2.341">
                                            </path>
                                            <path
                                                d="M21.119 42.422v-2.027h1.836v2.027h1.111V37.58h-1.111v1.859h-1.836V37.58H20v4.842z">
                                            </path>
                                            <path
                                                d="M27.943 37.5c-1.455 0-2.398 1.086-2.398 2.535c0 1.381.855 2.465 2.318 2.465c1.44 0 2.422-.963 2.422-2.549c0-1.338-.828-2.451-2.342-2.451m-.021 4.131c-.746 0-1.199-.668-1.199-1.617c0-.939.438-1.645 1.191-1.645c.771 0 1.193.748 1.193 1.617c0 .942-.431 1.645-1.185 1.645">
                                            </path>
                                            <path d="M31.17 38.498h1.324v3.924h1.117v-3.924h1.348v-.918H31.17z"></path>
                                            <path
                                                d="M37.404 40.373h1.813v-.891h-1.813v-1.003h1.922v-.899h-3.045v4.842h3.147v-.899h-2.024z">
                                            </path>
                                            <path d="M42.039 37.58H40.92v4.842H44v-.92h-1.961z"></path>

                                            <path
                                                d="M34 24h-4c-.551 0-1 .45-1 1v8c0 .551.449 1 1 1h4c.549 0 1-.449 1-1v-8c0-.55-.451-1-1-1">
                                            </path>
                                            <path
                                                d="M30 20c.549 0 1-.45 1-1v-4c0-.55-.451-1-1-1c-.551 0-1 .45-1 1v4c0 .55.449 1 1 1">
                                            </path>
                                            <path
                                                d="M34 14c-.551 0-1 .45-1 1v4c0 .55.449 1 1 1c.549 0 1-.45 1-1v-4c0-.55-.451-1-1-1">
                                            </path>
                                            <path
                                                d="M23 28h3c.549 0 1-.449 1-1v-2c0-.55-.451-1-1-1h-3c-.551 0-1 .45-1 1v2c0 .55.449 1 1 1">
                                            </path>
                                            <path
                                                d="M27 33v-2c0-.549-.451-1-1-1h-3c-.551 0-1 .451-1 1v2c0 .551.449 1 1 1h3c.549 0 1-.449 1-1">
                                            </path>
                                            <path
                                                d="M38 28h3c.549 0 1-.449 1-1v-2c0-.55-.451-1-1-1h-3c-.551 0-1 .45-1 1v2c0 .55.449 1 1 1">
                                            </path>
                                            <path
                                                d="M38 34h3c.549 0 1-.449 1-1v-2c0-.549-.451-1-1-1h-3c-.551 0-1 .451-1 1v2c0 .551.449 1 1 1">
                                            </path>
                                        </svg>
                                    </div>
                                    <h6>Hotels</h6>
                                    <p class="list">
                                        <?php
                                        $sql = "SELECT * FROM `listings` WHERE `listing_cat` = 'Hotels' AND `status` = 'approved'";
                                        $result = mysqli_query($conn, $sql);
                                        $count = mysqli_num_rows($result);
                                        $formattedCount = str_pad($count, 2, '0', STR_PAD_LEFT);

                                        if ($count === 1) {
                                            echo "<span>" . $formattedCount . "</span> Listing";
                                        } else {
                                            echo "<span>" . $formattedCount . "</span> Listings";
                                        }
                                        ?>
                                    </p>
                                </div>
                            </a>
                        </div>

                        <div class="col-md-4 col-lg-2 category-responsive text-center d-flex align-items-stretch"
                            data-aos="fade-up" data-aos-delay="100" data-aos-duration="1000">
                            <a href="shopping.php" class="category-wrap">
                                <div class="category-block">
                                    <div class="category-img">
                                        <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                                            xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 428 428"
                                            xml:space="preserve">
                                            <g>
                                                <g>
                                                    <path
                                                        d="M358.324,288.615c-2.93-2.929-7.678-2.929-10.607,0l-43.066,43.066c-2.929,2.929-2.929,7.678,0,10.606 c1.465,1.464,3.385,2.197,5.304,2.197c1.919,0,3.839-0.732,5.304-2.197l43.066-43.066 C361.253,296.292,361.253,291.543,358.324,288.615z" />
                                                </g>
                                            </g>
                                            <g>
                                                <g>
                                                    <path
                                                        d="M358.322,331.679c-1.399-1.39-3.329-2.19-5.3-2.19c-1.979,0-3.91,0.8-5.31,2.19c-1.391,1.4-2.19,3.33-2.19,5.31 c0,1.97,0.8,3.9,2.19,5.3c1.399,1.4,3.33,2.2,5.31,2.2c1.971,0,3.9-0.8,5.3-2.2c1.4-1.4,2.2-3.33,2.2-5.3 C360.522,335.009,359.722,333.079,358.322,331.679z" />
                                                </g>
                                            </g>
                                            <g>
                                                <g>
                                                    <path
                                                        d="M315.253,288.619c-1.391-1.4-3.32-2.2-5.3-2.2c-1.971,0-3.91,0.8-5.301,2.2c-1.399,1.39-2.199,3.32-2.199,5.3 c0,1.97,0.8,3.91,2.199,5.3c1.391,1.4,3.33,2.2,5.301,2.2c1.979,0,3.909-0.8,5.3-2.2c1.399-1.4,2.2-3.33,2.2-5.3 C317.453,291.939,316.652,290.009,315.253,288.619z" />
                                                </g>
                                            </g>
                                            <g>
                                                <g>
                                                    <path
                                                        d="M411.464,187.462c-0.195-3.995-3.491-7.133-7.491-7.133H368.69v-13.825c0-21.146-17.203-38.35-38.35-38.35 c-6.467,0-12.562,1.616-17.912,4.455c-5.469-2.912-11.604-4.458-17.98-4.458c-21.146,0-38.35,17.204-38.35,38.35v13.829h-16.166 V93.76c0-4.142-3.357-7.5-7.5-7.5h-27.779V47.5c0-4.142-3.357-7.5-7.5-7.5h-4.018c1.924-3.753,3.018-8,3.018-12.5 c0-15.164-12.337-27.5-27.5-27.5c-7.875,0-14.982,3.332-20,8.655C143.635,3.332,136.528,0,128.653,0 c-15.163,0-27.5,12.336-27.5,27.5c0,4.5,1.093,8.747,3.018,12.5H94.153c-4.143,0-7.5,3.358-7.5,7.5v38.76H12.645 c-4.143,0-7.5,3.358-7.5,7.5V420.5c0,4.142,3.357,7.5,7.5,7.5H83.6c0.001,0,0.003,0,0.004,0h161.724h97.406h72.621 c2.053,0,4.016-0.841,5.432-2.328c1.415-1.486,2.16-3.488,2.06-5.538L411.464,187.462z M330.34,143.154 c12.876,0,23.35,10.474,23.35,23.35v13.825h-20.893V166.5c0-8.325-2.633-16.24-7.499-22.792 C326.923,143.349,328.609,143.154,330.34,143.154z M306.991,166.504c0-5.672,2.035-10.877,5.41-14.927 c3.492,4.179,5.397,9.402,5.397,14.923v13.829h-10.807V166.504z M271.098,166.5c0.001-12.875,10.475-23.35,23.35-23.35 c1.729,0,3.428,0.193,5.076,0.557c-4.73,6.377-7.533,14.265-7.533,22.797v13.825h-20.893V166.5z M189.653,55v31.26h-23V55h2 H189.653z M156.153,27.5c0-6.893,5.608-12.5,12.5-12.5c6.893,0,12.5,5.607,12.5,12.5s-5.607,12.5-12.5,12.5h-12.5V27.5z  M128.653,15c6.893,0,12.5,5.607,12.5,12.5V40h-12.5c-6.892,0-12.5-5.607-12.5-12.5S121.76,15,128.653,15z M101.653,55h27h20h3 v31.26h-14.529h-0.001h-35.47V55z M83.603,195c-4.142,0-7.5,3.358-7.5,7.5V413H20.145V101.26h74.008h35.47V195H83.603z  M202.294,413H91.103V210h121.122L202.294,413z M213.329,187.462l-0.37,7.538h-68.335v-93.74h52.529h27.779v79.069h-4.112 C216.82,180.329,213.524,183.468,213.329,187.462z M238.185,413h-20.872l10.648-217.671h20.872L238.185,413z M342.733,413h-89.53 l10.647-217.671h28.141v17.45c0,4.142,3.357,7.5,7.5,7.5s7.5-3.358,7.5-7.5v-17.45h46.699v17.45c0,4.142,3.357,7.5,7.5,7.5 s7.5-3.358,7.5-7.5v-17.45h28.141L407.478,413H342.733z" />
                                                </g>
                                            </g>
                                        </svg>
                                    </div>
                                    <h6>Shopping</h6>
                                    <p class="list">
                                        <?php
                                        $sql = "SELECT * FROM `listings` WHERE `listing_cat` = 'Shopping' AND `status` = 'approved'";
                                        $result = mysqli_query($conn, $sql);
                                        $count = mysqli_num_rows($result);
                                        $formattedCount = str_pad($count, 2, '0', STR_PAD_LEFT);

                                        if ($count === 1) {
                                            echo "<span>" . $formattedCount . "</span> Listing";
                                        } else {
                                            echo "<span>" . $formattedCount . "</span> Listings";
                                        }
                                        ?>
                                    </p>
                                </div>
                            </a>
                        </div>

                        <div class="col-md-4 col-lg-2 category-responsive text-center d-flex align-items-stretch"
                            data-aos="fade-up" data-aos-delay="100" data-aos-duration="1000">
                            <a href="beauty.php" class="category-wrap">
                                <div class="category-block">
                                    <div class="category-img">
                                        <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                                            xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 496 496"
                                            xml:space="preserve">
                                            <g>
                                                <g>
                                                    <g>
                                                        <path
                                                            d="M320,40c-74.992,0-136,61.008-136,136s61.008,136,136,136s136-61.008,136-136S394.992,40,320,40z M320,296 c-66.168,0-120-53.832-120-120S253.824,56,320,56s120,53.832,120,120S386.168,296,320,296z" />
                                                        <rect x="315.998" y="111.898"
                                                            transform="matrix(-0.6459 -0.7634 0.7634 -0.6459 395.845 543.6352)"
                                                            width="16" height="136.238" />
                                                        <rect x="246.397" y="124.018"
                                                            transform="matrix(0.7894 -0.6139 0.6139 0.7894 -19.5454 207.0645)"
                                                            width="91.217" height="16" />
                                                        <path
                                                            d="M48,0H32v24H0v16h32v16H0v16h32v16H0v16h32v16H0v16h32v16H0v16h32v168h-8c-8.824,0-16,7.176-16,16v22.24 c-4.872,4.4-8,10.696-8,17.76v80c0,13.232,10.768,24,24,24h32c13.232,0,24-10.768,24-24v-80c0-7.064-3.128-13.36-8-17.76V352 c0-8.824-7.184-16-16-16h-8V168h32v-16H48v-16h32v-16H48v-16h32V88H48V72h32V56H48V40h32V24H48V0z M64,392v80 c0,4.416-3.584,8-8,8H24c-4.416,0-8-3.584-8-8v-80c0-4.416,3.584-8,8-8h32C60.416,384,64,387.584,64,392z M56,352v16H24v-16H56z" />
                                                        <path
                                                            d="M496,176C496,78.952,417.048,0,320,0c-95.488,0-173.4,76.448-175.88,171.352l-2.808-4.208 c-3.936-5.904-11.168-8.488-17.944-6.44c-6.8,2.056-11.368,8.208-11.368,15.312v106.208c-4.76,2.776-8,7.88-8,13.776v18.224 c-4.76,2.776-8,7.88-8,13.776v152c0,8.824,7.176,16,16,16h48c8.816,0,16-7.176,16-16V328c0-5.896-3.24-11-8-13.776V296 c0-5.896-3.24-11-8-13.776v-33.04c26.776,58.312,84.48,99.576,152,102.616V480H200v16h120h120v-16H328V351.8 C421.344,347.6,496,270.368,496,176z M128,176.016l16,24V280h-16V176.016z M120,296h32v16h-32V296z M160.016,480H112V328h48 L160.016,480z M320,336c-88.224,0-160-71.776-160-160S231.776,16,320,16s160,71.776,160,160S408.224,336,320,336z" />
                                                    </g>
                                                </g>
                                            </g>
                                        </svg>
                                    </div>
                                    <h6>Beauty</h6>
                                    <p class="list">
                                        <?php
                                        $sql = "SELECT * FROM `listings` WHERE `listing_cat` = 'Beauty' AND `status` = 'approved'";
                                        $result = mysqli_query($conn, $sql);
                                        $count = mysqli_num_rows($result);
                                        $formattedCount = str_pad($count, 2, '0', STR_PAD_LEFT);

                                        if ($count === 1) {
                                            echo "<span>" . $formattedCount . "</span> Listing";
                                        } else {
                                            echo "<span>" . $formattedCount . "</span> Listings";
                                        }
                                        ?>
                                    </p>
                                </div>
                            </a>
                        </div>

                        <div class="col-md-4 col-lg-2 category-responsive text-center d-flex align-items-stretch"
                            data-aos="fade-up" data-aos-delay="100" data-aos-duration="1000">
                            <a href="fitness.php" class="category-wrap">
                                <div class="category-block">
                                    <div class="category-img">
                                        <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg"
                                            xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 216.61 216.61"
                                            xml:space="preserve">
                                            <g>
                                                <path
                                                    d="M103.468,118.835c0.597,25.136-19.373,46.095-44.533,46.686c-0.362,0.007-0.719,0.013-1.078,0.013 c-24.667,0-45.026-19.766-45.604-44.549c-0.438-18.788,10.948-36.196,28.339-43.318c1.571-0.642,3.41,0.113,4.059,1.702 c0.648,1.59-0.119,3.413-1.696,4.064c-15.016,6.15-24.847,21.185-24.466,37.399c0.508,21.708,18.642,39.19,40.31,38.447 c21.699-0.518,38.946-18.596,38.435-40.304c-0.046-1.718,1.327-3.148,3.041-3.19C102.125,115.913,103.422,117.113,103.468,118.835z  M48.97,120.133c-0.073-3.801,2.253-7.088,5.574-8.434l-0.804-34.2c-0.042-1.72,1.321-3.151,3.036-3.194 c1.717,0.119,3.157,1.322,3.2,3.044l0.804,34.246c3.432,1.229,5.861,4.384,5.955,8.104c0.061,2.362-0.81,4.615-2.457,6.333 c-1.641,1.717-3.833,2.697-6.211,2.752c-0.024,0-0.174,0-0.198,0C53.01,128.792,49.107,124.986,48.97,120.133z M60.5,119.852 c-0.031-1.424-1.209-2.588-2.637-2.588c-1.522,0.037-2.682,1.249-2.658,2.723c0.045,1.412,1.229,2.575,2.664,2.575v3.118 l0.061-3.118c0.7-0.012,1.361-0.311,1.839-0.821C60.256,121.216,60.512,120.553,60.5,119.852z M165.254,127.848 c-32.145-14.328-51.398,5.82-51.593,6.027c-1.17,1.255-1.097,3.24,0.158,4.409c0.608,0.554,1.363,0.834,2.131,0.834 c0.828,0,1.675-0.341,2.277-0.992c0.158-0.171,16.368-17.099,44.5-4.573c1.546,0.7,3.409,0,4.116-1.577 C167.538,130.393,166.832,128.547,165.254,127.848z M215.051,130.009c1.633,4.451,1.571,8.513-0.17,12.093 c-3.203,6.589-10.997,8.72-11.32,8.817c-19.826,8.452-35.725,11.174-47.988,11.174c-16.367,0-26.238-4.841-30.27-7.368 c-11.617,8.038-24.274,4.281-24.84,4.11c-0.612-0.183-1.062-0.566-1.428-1.022c-10.108,11.252-24.6,18.522-40.886,18.9 l-1.367,0.012c-30.57,0-56.035-24.887-56.766-55.472c-0.359-15.175,5.222-29.569,15.695-40.542 c8.705-9.118,19.936-14.909,32.175-16.836l-0.137-6.378c-3.325-1.727-5.294-4.439-5.349-7.538 c-0.076-2.844,1.538-5.557,4.408-7.438c2.469-1.616,5.712-2.551,9.134-2.633c3.422-0.048,6.689,0.701,9.229,2.202 c2.975,1.753,4.701,4.384,4.777,7.228c0.073,3.087-1.754,5.891-4.99,7.776l0.158,6.713c7.188,1.081,13.959,3.51,20.052,7.072 l2.655-3.443c-0.761-2.25-0.511-4.481,0.768-6.135c1.315-1.708,3.474-2.563,6.031-2.296c1.994,0.201,4.101,1.087,5.921,2.497 c4.259,3.297,5.663,8.348,3.224,11.508c-1.278,1.653-3.446,2.426-5.736,2.302l-2.512,3.254c4.826,4.521,8.86,9.871,11.889,15.808 c5.797-2.089,25.915-7.31,38.989,14.623c4.42-1.991,13.329-5.108,22.115-3.015c-2.861-12.421,0.231-25.863,1.754-31.219 c-8.622,1.084-17.671,0.673-21.885,0.049c-1.145-0.167-2.594-1.309-2.978-2.411c-0.14-0.414-0.365-1.011-0.164-1.985 c-1.376,0.089-2.74-0.055-3.849-0.608c-0.597-0.296-2.18-1.081-3.068-6.51c-0.512-3.105-0.598-7.322,1.881-9.791 c5.03-5.039,31.292-12.002,37.71-7.733c7.855,5.24,16.398,18.508,23.784,30.537l0.652,1.072 C220.648,107.108,216.763,125.059,215.051,130.009z M93.896,69.754l-3.523,4.569c0.119,0.083,0.226,0.165,0.35,0.25l3.504-4.558 c0.792-1.029,2.125-1.446,3.379-1.078c0.804,0.25,1.258,0.18,1.364,0.131c-0.186-0.042-0.685-1.416-2.256-2.63 c-1.373-1.059-2.646-1.285-3.2-1.23c0.131,0.088,0.244,0.481,0.569,0.999C94.77,67.309,94.688,68.725,93.896,69.754z  M54.109,63.245c0.447-0.018,0.91-0.088,1.364-0.101l1.367-0.012c0.679,0,1.339,0.076,2.006,0.101l-0.18-7.968 c-0.03-1.297,0.749-2.484,1.945-2.969c2.144-0.868,3.093-2.07,3.082-2.819c-0.021-0.643-0.649-1.391-1.696-2.006 c-1.528-0.904-3.748-1.397-5.918-1.337c-2.232,0.052-4.378,0.643-5.849,1.61c-1.023,0.667-1.629,1.446-1.607,2.083 c0.024,0.755,1.026,1.912,3.215,2.673c1.23,0.426,2.07,1.571,2.095,2.868L54.109,63.245z M107.377,118.745 c-0.648-27.228-23.322-49.383-50.537-49.383l-1.218,0.012C42.117,69.693,29.549,75.25,20.22,85.02 c-9.323,9.77-14.285,22.588-13.962,36.093c0.648,27.237,23.321,49.396,50.531,49.396l1.218-0.012 C85.87,169.838,108.026,146.626,107.377,118.745z M208.908,128.639c0.061-0.128,7.423-16.654-11.874-47.985l-0.657-1.078 c-5.085-8.281-14.56-23.696-21.934-28.609c-3.58-1.942-26.183,3.288-29.849,6.954c-0.646,0.816-0.292,4.348,0.292,6.324 c1.395-0.137,4.08-0.956,6.284-1.976c1.364-0.652,2.978-0.219,3.849,0.996c0.877,1.208,0.755,2.874-0.256,3.967 c-0.548,0.572-1.108,1.209-1.596,1.821c7.502,0.5,19.717,0.125,26.707-2.975c1.559-0.697,3.422,0.012,4.116,1.589 c0.694,1.571-0.018,3.416-1.595,4.116c-1.632,0.722-3.483,1.279-5.407,1.757c-0.013,0.292-0.013,0.588-0.116,0.88 c-0.073,0.207-6.844,20.496-0.736,35.01c5.468,3.787,9.426,10.065,11.739,18.821c0.438,1.662-0.554,3.367-2.204,3.806 c-0.28,0.067-0.548,0.109-0.804,0.109c-1.389,0-2.643-0.926-3.008-2.319c-2.338-8.848-6.448-14.554-12.209-16.958 c-10.248-4.263-22.847,2.892-22.986,2.959c-0.719,0.426-1.596,0.535-2.436,0.292c-0.816-0.237-1.498-0.798-1.894-1.559 c-10.645-20.533-27.39-16.35-32.429-14.511c2.202,5.809,3.541,12.032,3.699,18.541c0.293,12.836-3.754,24.753-10.738,34.435 c2.415,0.585,12.004,2.302,20.383-4.548c1.157-0.938,2.813-0.938,3.977,0.018c0.201,0.171,23.352,18.256,74.306-3.459 c0.451-0.152,5.827-1.68,7.745-5.687c1.078-2.204,0.914-4.933-0.438-8.104C208.463,130.411,208.5,129.461,208.908,128.639z" />
                                            </g>
                                        </svg>
                                    </div>
                                    <h6>Fitness</h6>
                                    <p class="list">
                                        <?php
                                        $sql = "SELECT * FROM `listings` WHERE `listing_cat` = 'Fitness' AND `status` = 'approved'";
                                        $result = mysqli_query($conn, $sql);
                                        $count = mysqli_num_rows($result);
                                        $formattedCount = str_pad($count, 2, '0', STR_PAD_LEFT);

                                        if ($count === 1) {
                                            echo "<span>" . $formattedCount . "</span> Listing";
                                        } else {
                                            echo "<span>" . $formattedCount . "</span> Listings";
                                        }
                                        ?>
                                    </p>
                                </div>
                            </a>
                        </div>

                        <div class="col-md-4 col-lg-2 category-responsive text-center d-flex align-items-stretch"
                            data-aos="fade-up" data-aos-delay="100" data-aos-duration="1000">
                            <a href="hookah_bars.php" class="category-wrap">
                                <div class="category-block">
                                    <div class="category-img">
                                        <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                                            xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 512 512"
                                            xml:space="preserve">
                                            <g>
                                                <g>
                                                    <path
                                                        d="M436.774,32.417c-0.35-4.634-4.389-8.108-9.024-7.757c-4.633,0.35-8.107,4.39-7.757,9.024 c1.795,23.766-0.748,58.43-23.058,88.606c-22.341,30.219-54.028,40.732-71.052,46.381c-25.21,8.363-48.352,10.542-69.817,9.979 c1.881-4.059,2.939-8.576,2.939-13.337c0-14.823-10.186-27.312-23.925-30.836v-10.971h44.928 c17.013,0,30.853-13.841,30.853-30.853v-8.975c0-4.648-3.767-8.414-8.414-8.414h-40.949l16.837-54.309 c1.537-4.958,0.653-10.199-2.427-14.377c-3.08-4.178-7.823-6.574-13.014-6.574h-44.643c-4.647,0-8.414,3.766-8.414,8.414 c0,4.648,3.767,8.414,8.414,8.414h43.744l-18.115,58.433h-34.692l-24.79-69.67c-1.557-4.378-6.372-6.66-10.748-5.107 c-4.379,1.558-6.664,6.371-5.107,10.749l22.783,64.026H151.55c-4.647,0-8.414,3.766-8.414,8.414v8.975 c0,17.012,13.84,30.853,30.853,30.853h44.264v11.26c-13.218,3.868-22.905,16.095-22.905,30.548c0,2.343,0.261,4.625,0.745,6.825 c-33.415-4.715-62.472-7.003-86.439,15.173c-6.523,6.037-21.919,22.699-23.986,46.974c-0.14,1.644-3.061,40.486,28.382,61.555 c12.524,8.392,25.511,10.719,36.061,10.719c4.212,0,8.032-0.371,11.281-0.875c2.441,7.023,7.351,12.896,13.687,16.582 c-31.123,17.28-52.237,50.487-52.237,88.538c0,18.833,5.246,37.127,14.972,52.956h-23.671c-21.723,0-39.396,17.674-39.396,39.396 v0.432c0,4.648,3.767,8.414,8.414,8.414h287.676c4.647,0,8.414-3.766,8.414-8.414v-0.432c0-21.723-17.672-39.396-39.396-39.396 H158.359c-12.076-14.965-18.689-33.624-18.689-52.956c0-46.521,37.848-84.369,84.369-84.369c46.521,0,84.37,37.848,84.37,84.369 c0,8.859-1.364,17.581-4.056,25.923c-1.426,4.422,1.003,9.164,5.426,10.59c4.424,1.427,9.164-1.003,10.591-5.426 c3.229-10.014,4.867-20.474,4.867-31.088c0-36.91-19.866-69.262-49.463-86.941c10.899-4.765,18.539-15.645,18.539-28.281 c0-13.885-9.222-25.654-21.862-29.512c-2.152-13.881-10.537-25.722-22.192-32.585c5.115-5.65,8.235-13.137,8.235-21.34 c0-6.15-1.758-11.895-4.792-16.768c23.5,0.765,49.414-1.422,77.478-10.732c17.455-5.791,53.774-17.842,79.285-52.348 C437.19,96.149,438.42,54.202,436.774,32.417z M154.351,480.586c0.036,0,0.071,0.006,0.107,0.006c0.042,0,0.083-0.004,0.125-0.006 h185.274c9.634,0,17.878,6.068,21.109,14.585H93.034c3.231-8.516,11.475-14.585,21.109-14.585H154.351z M173.988,106.677 c-7.732,0-14.024-6.292-14.024-14.024v-0.561h43.287h46.83h43.952v0.561c0,7.733-6.291,14.024-14.024,14.024H173.988z  M225.083,150.474c0.514,0.098,1.042,0.154,1.584,0.154c0.682,0,1.342-0.09,1.978-0.243c7.584,0.74,13.533,7.15,13.533,14.927 c0,4.954-2.426,9.342-6.139,12.075c-6.957-0.669-13.732-1.526-20.343-2.441c-2.192-2.608-3.519-5.968-3.519-9.634 C212.176,157.752,217.802,151.498,225.083,150.474z M226.667,197.14c8.272,0,15.001,6.729,15.001,15.001 c0,8.271-6.729,15-15.001,15c-8.271,0.001-15-6.729-15-15C211.667,203.87,218.396,197.14,226.667,197.14z M160.454,288.755 c-8.067,1.47-23.181,2.386-37.036-6.898c-23.294-15.609-21.008-45.845-20.982-46.149c1.566-18.411,13.563-31.344,18.647-36.048 c20.028-18.533,45.548-14.818,80.869-9.68c0.552,0.081,1.112,0.162,1.668,0.242c-5.436,5.713-8.782,13.43-8.782,21.919 c0,8.326,3.217,15.911,8.467,21.588c-11.431,6.899-19.634,18.625-21.76,32.336C171.041,269.272,162.905,277.945,160.454,288.755z  M263.461,281.556c7.733,0,14.024,6.291,14.024,14.024s-6.291,14.024-14.024,14.024h-72.925c-7.733,0-14.024-6.291-14.024-14.024 s6.291-14.024,14.024-14.024H263.461z M254.927,264.727h-55.855c3.62-11.996,14.768-20.756,27.927-20.756 S251.305,252.733,254.927,264.727z" />
                                                </g>
                                            </g>
                                        </svg>
                                    </div>
                                    <h6>Hookah Bars</h6>
                                    <p class="list">
                                        <?php
                                        $sql = "SELECT * FROM `listings` WHERE `listing_cat` = 'Hookah Bars' AND `status` = 'approved'";
                                        $result = mysqli_query($conn, $sql);
                                        $count = mysqli_num_rows($result);
                                        $formattedCount = str_pad($count, 2, '0', STR_PAD_LEFT);

                                        if ($count === 1) {
                                            echo "<span>" . $formattedCount . "</span> Listing";
                                        } else {
                                            echo "<span>" . $formattedCount . "</span> Listings";
                                        }
                                        ?>
                                    </p>
                                </div>
                            </a>
                        </div>

                        <div class="col-md-4 col-lg-2 category-responsive text-center d-flex align-items-stretch"
                            data-aos="fade-up" data-aos-delay="100" data-aos-duration="1000">
                            <a href="games.php" class="category-wrap">
                                <div class="category-block">
                                    <div class="category-img">
                                        <svg version="1.1" id="_x32_" xmlns="http://www.w3.org/2000/svg"
                                            xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 512 512"
                                            xml:space="preserve">
                                            <g>
                                                <path class="st0"
                                                    d="M389.488,226.899H122.512C54.854,226.899,0,281.746,0,349.412c0,67.658,54.854,122.512,122.512,122.512 c42.653,0,80.199-21.812,102.135-54.854h62.706c21.936,33.042,59.49,54.854,102.135,54.854C457.154,471.924,512,417.07,512,349.412 C512,281.746,457.154,226.899,389.488,226.899z M389.488,440.332c-30.535,0-58.881-15.226-75.818-40.74l-9.372-14.115h-16.945 h-62.706h-16.945l-9.371,14.115c-16.938,25.514-45.282,40.74-75.819,40.74c-50.134,0-90.92-40.786-90.92-90.92 c0-50.134,40.786-90.921,90.92-90.921h266.976c50.134,0,90.92,40.786,90.92,90.921 C480.408,399.545,439.622,440.332,389.488,440.332z" />
                                                <path class="st0"
                                                    d="M268.927,110.897c0-2.46,0.486-4.72,1.366-6.803c1.311-3.116,3.548-5.8,6.332-7.69 c2.8-1.89,6.085-2.97,9.757-2.97c2.437,0,4.705,0.494,6.788,1.373c3.116,1.311,5.799,3.533,7.689,6.333 c1.882,2.8,2.97,6.085,2.97,9.756c0,5.893,1.203,11.593,3.394,16.752c3.278,7.744,8.716,14.292,15.58,18.928 c6.849,4.643,15.21,7.374,24.08,7.366c5.916,0,11.616-1.211,16.768-3.394c7.728-3.278,14.284-8.716,18.92-15.58 c4.643-6.857,7.366-15.21,7.366-24.072v-70.82h-25.607v70.82c0,2.437-0.478,4.705-1.357,6.78c-1.319,3.124-3.556,5.808-6.333,7.69 c-2.808,1.881-6.093,2.969-9.757,2.969c-2.436,0-4.705-0.486-6.787-1.365c-3.116-1.311-5.8-3.548-7.682-6.333 c-1.889-2.8-2.977-6.1-2.992-9.741c0.016-5.924-1.188-11.616-3.378-16.768c-3.279-7.744-8.716-14.292-15.58-18.928 c-6.864-4.651-15.21-7.374-24.08-7.366c-5.909-0.008-11.608,1.203-16.768,3.394c-7.743,3.263-14.292,8.716-18.927,15.58 c-4.644,6.857-7.359,15.21-7.359,24.088v104.71h25.599V110.897z" />
                                                <polygon class="st0"
                                                    points="138.363,293.338 105.452,293.338 105.452,331.124 67.658,331.124 67.658,364.051 105.452,364.051  105.452,401.829 138.363,401.829 138.363,364.051 176.156,364.051 176.156,331.124 138.363,331.124 	" />
                                                <path class="st0"
                                                    d="M375.126,289.698c-7.937,7.929-7.921,20.794,0,28.723c7.936,7.929,20.794,7.929,28.716,0 c7.936-7.929,7.936-20.794,0-28.723C395.92,281.769,383.063,281.769,375.126,289.698z" />
                                                <path class="st0"
                                                    d="M329.774,335.05c-7.929,7.929-7.929,20.794,0,28.723c7.937,7.929,20.794,7.929,28.723,0 c7.93-7.929,7.93-20.794,0-28.731C350.568,327.121,337.711,327.121,329.774,335.05z" />
                                                <path class="st0"
                                                    d="M375.126,380.402c-7.937,7.929-7.937,20.786,0,28.715c7.936,7.929,20.794,7.929,28.716,0.008 c7.936-7.937,7.936-20.794,0-28.739C395.92,372.457,383.063,372.473,375.126,380.402z" />
                                                <path class="st0"
                                                    d="M449.193,335.05c-7.921-7.929-20.786-7.929-28.723,0c-7.937,7.929-7.937,20.794,0,28.723 c7.937,7.929,20.802,7.929,28.723,0C457.123,355.844,457.123,342.979,449.193,335.05z" />
                                            </g>
                                        </svg>
                                    </div>
                                    <h6>Games</h6>
                                    <p class="list">
                                        <?php
                                        $sql = "SELECT * FROM `listings` WHERE `listing_cat` = 'Games' AND `status` = 'approved'";
                                        $result = mysqli_query($conn, $sql);
                                        $count = mysqli_num_rows($result);
                                        $formattedCount = str_pad($count, 2, '0', STR_PAD_LEFT);

                                        if ($count === 1) {
                                            echo "<span>" . $formattedCount . "</span> Listing";
                                        } else {
                                            echo "<span>" . $formattedCount . "</span> Listings";
                                        }
                                        ?>
                                    </p>
                                </div>
                            </a>
                        </div>

                        <div class="col-md-4 col-lg-2 category-responsive text-center d-flex align-items-stretch"
                            data-aos="fade-up" data-aos-delay="100" data-aos-duration="1000">
                            <a href="places.php" class="category-wrap">
                                <div class="category-block">
                                    <div class="category-img">
                                        <svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"
                                            xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" role="img"
                                            class="iconify iconify--gis" preserveAspectRatio="xMidYMid meet">
                                            <path d="M68.913 48.908l-.048.126c.015-.038.027-.077.042-.115l.006-.011z">
                                            </path>
                                            <path
                                                d="M50.002 0C33.524 0 20 13.48 20 29.922c0 6.372 2.04 12.31 5.48 17.174l-.123-.192l19.206 33.198l.097.127c.774 1.01 1.54 1.824 2.467 2.437c.927.613 2.114 1 3.28.883c2.33-.234 3.684-1.844 4.935-3.54l.078-.105L76.594 43.87l.017-.03c.507-.915.874-1.838 1.188-2.732A29.578 29.578 0 0 0 80 29.922C80 13.48 66.48 0 50.002 0zm0 5C63.756 5 75 16.218 75 29.922a24.55 24.55 0 0 1-1.84 9.332l-.025.062l-.022.065c-.263.757-.549 1.444-.879 2.04L51.275 77.087c-.927 1.221-1.575 1.51-1.367 1.488c.107-.01.206.074-.023-.078c-.219-.145-.666-.57-1.196-1.246L29.63 44.3l-.067-.093C26.69 40.147 25 35.23 25 29.922C25 16.219 36.248 5 50.002 5zm0 7.85c-9.462 0-17.115 7.626-17.115 17.072c0 9.446 7.654 17.072 17.115 17.072c9.461 0 17.111-7.627 17.111-17.072c0-9.446-7.65-17.072-17.111-17.072zm0 5c6.817 0 12.111 5.285 12.111 12.072c0 6.787-5.293 12.072-12.111 12.072c-6.818 0-12.115-5.286-12.115-12.072c0-6.787 5.298-12.072 12.115-12.072z">
                                            </path>
                                            <path
                                                d="M34.006 69.057C19.88 71.053 10 75.828 10 82.857C10 92.325 26.508 100 50 100s40-7.675 40-17.143c0-7.029-9.879-11.804-24.004-13.8l-1.957 3.332C74.685 73.866 82 76.97 82 80.572c0 5.05-14.327 9.143-32 9.143c-17.673 0-32-4.093-32-9.143c-.001-3.59 7.266-6.691 17.945-8.174c-.645-1.114-1.294-2.226-1.94-3.341z">
                                            </path>
                                        </svg>
                                    </div>
                                    <h6>Places</h6>
                                    <p class="list">
                                        <?php
                                        $sql = "SELECT * FROM `listings` WHERE `listing_cat` = 'Places' AND `status` = 'approved'";
                                        $result = mysqli_query($conn, $sql);
                                        $count = mysqli_num_rows($result);
                                        $formattedCount = str_pad($count, 2, '0', STR_PAD_LEFT);

                                        if ($count === 1) {
                                            echo "<span>" . $formattedCount . "</span> Listing";
                                        } else {
                                            echo "<span>" . $formattedCount . "</span> Listings";
                                        }
                                        ?>
                                    </p>
                                </div>
                            </a>
                        </div>

                        <div class="col-md-4 col-lg-2 category-responsive text-center d-flex align-items-stretch"
                            data-aos="fade-up" data-aos-delay="100" data-aos-duration="1000">
                            <a href="shows.php" class="category-wrap">
                                <div class="category-block">
                                    <div class="category-img">
                                        <svg version="1.1" id="_x32_" xmlns="http://www.w3.org/2000/svg"
                                            xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 512 512"
                                            xml:space="preserve">
                                            <g>
                                                <path
                                                    d="M293.686,333.324c5.976,52.529,58.497,59.684,74.014,59.684c15.521,0,68.042-7.155,74.01-59.684 c-9.545,8.358-40.582,22.691-74.01,22.691C334.276,356.014,303.239,341.681,293.686,333.324z" />
                                                <path
                                                    d="M312.681,272.069c12.31,1.33,23.671,5.338,33.591,11.37c0.394-17.695-12.824-33.041-30.747-34.969 c-17.827-1.92-33.926,10.126-37.423,27.344C288.894,272.189,300.622,270.778,312.681,272.069z" />
                                                <path
                                                    d="M389.129,283.439c9.919-6.032,21.285-10.039,33.587-11.37c12.067-1.291,23.786,0.12,34.579,3.745 c-3.498-17.218-19.596-29.264-37.42-27.344C401.948,250.398,388.73,265.744,389.129,283.439z" />
                                                <path
                                                    d="M511.724,171.919c0,0-0.02-0.216-0.036-0.327l-0.02-0.104l-0.02-0.144c-0.948-6.86-5.975-12.389-13.05-13.106 c-5.765-0.59-11.586,2.35-17.119,3.673c-11.405,2.693-22.97,4.74-34.535,6.462c-30.085,4.478-60.56,6.549-90.964,5.832 c-23.604-0.55-47.223-2.621-70.552-6.278c-8.7-36.101-23.006-95.259-27.058-112.023c-0.82-3.402-1.43-8.724-8.29-3.792 c-4.96,3.593-54.665,28.874-118.249,44.243C68.247,111.724,12.475,111.916,6.42,111c-8.346-1.275-6.466,3.744-5.641,7.146 C5.133,136.16,21.294,203.039,29.7,237.777c20.496,84.87,111.994,128.117,164.213,115.489c11.242-2.717,22.918-8.573,34.125-16.939 c11.282,46.108,43.136,86.902,84.714,109.784c13.413,7.378,28.077,12.979,43.311,15.027c14.935,2.023,29.942-0.167,44.14-5.02 c27.938-9.56,52.665-28.213,71.624-50.649c18.481-21.886,31.731-48.227,37.172-76.408c2.506-12.964,2.848-25.934,2.848-39.073 V186.977C511.848,182.029,512.262,176.842,511.724,171.919z M61.156,185.351c11.358,0.972,23.082-0.398,34.487-4.493 c11.656-4.176,21.776-10.733,29.99-18.932c4.534,17.098-4.705,35.129-21.679,41.2C87.086,209.182,68.606,201.262,61.156,185.351z  M223.564,256.612c-13.808-2.852-33.539-3.991-52.84,0.685c-28.596,6.908-52.18,25.576-58.625,34.691 c-5.744-46.156,37.694-63.134,50.98-66.354c10.202-2.47,40.001-5.832,60.485,12.629V256.612z M231.835,159.648 c-1.932,3.928-4.581,7.466-7.844,10.406c-5.008,4.526-11.405,7.633-18.6,8.581c-17.875,2.327-34.324-9.489-38.108-26.794 c11.07,3.554,23.046,4.764,35.328,3.171c12.019-1.57,23.082-5.705,32.734-11.76C235.564,149.083,234.289,154.677,231.835,159.648z  M488.574,284.937c0,12.708,0.16,25.337-2.079,37.91c-1.933,10.867-5.247,21.448-9.72,31.519c-0.056,0.104-0.112,0.231-0.164,0.358 c-0.015,0.048-0.055,0.104-0.071,0.175c-0.088,0.183-0.164,0.374-0.251,0.557c-0.159,0.374-0.339,0.749-0.518,1.14 c0.048-0.144,0.123-0.263,0.179-0.415c-11.354,24.644-29.67,46.164-51.892,61.66c-12.302,8.589-26.109,15.593-40.813,18.939 c-13.732,3.155-27.284,1.825-40.57-2.701c-27.826-9.506-52.032-29.814-69.01-53.526c-13.286-18.564-22.277-40.076-25.496-62.576 c-0.88-6.103-1.326-12.302-1.326-18.516V184.149c14.274,3.513,28.83,6.102,43.438,8.07c9.314,1.275,18.676,2.295,27.986,3.14 c31.193,2.805,62.548,2.964,93.757,0.43c25.66-2.079,51.514-5.497,76.551-11.656V284.937z" />
                                            </g>
                                        </svg>
                                    </div>
                                    <h6>Shows</h6>
                                    <p class="list">
                                        <?php
                                        $sql = "SELECT * FROM `listings` WHERE `listing_cat` = 'Shows' AND `status` = 'approved'";
                                        $result = mysqli_query($conn, $sql);
                                        $count = mysqli_num_rows($result);
                                        $formattedCount = str_pad($count, 2, '0', STR_PAD_LEFT);

                                        if ($count === 1) {
                                            echo "<span>" . $formattedCount . "</span> Listing";
                                        } else {
                                            echo "<span>" . $formattedCount . "</span> Listings";
                                        }
                                        ?>
                                    </p>
                                </div>
                            </a>
                        </div>

                        <div class="col-md-4 col-lg-2 category-responsive text-center d-flex align-items-stretch"
                            data-aos="fade-up" data-aos-delay="100" data-aos-duration="1000">
                            <a href="movie_theater.php" class="category-wrap">
                                <div class="category-block">
                                    <div class="category-img">
                                        <svg version="1.1" id="Icons" xmlns="http://www.w3.org/2000/svg"
                                            xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 32 32"
                                            xml:space="preserve">
                                            <circle class="st1" cx="9" cy="7" r="5" />
                                            <circle class="st1" cx="19" cy="7" r="5" />
                                            <path class="st1"
                                                d="M21,25H7c-1.1,0-2-0.9-2-2v-6c0-1.1,0.9-2,2-2h14c1.1,0,2,0.9,2,2v6C23,24.1,22.1,25,21,25z" />
                                            <polygon class="st1" points="29,25 23,22 23,18 29,15 " />
                                            <polyline class="st1" points="10,31 14,25 18,31 " />
                                            <circle class="st1" cx="9" cy="7" r="1" />
                                            <circle class="st1" cx="19" cy="7" r="1" />
                                        </svg>
                                    </div>
                                    <h6>Movie Theaters</h6>
                                    <p class="list">
                                        <?php
                                        $sql = "SELECT * FROM `listings` WHERE `listing_cat` = 'Movie Theaters' AND `status` = 'approved'";
                                        $result = mysqli_query($conn, $sql);
                                        $count = mysqli_num_rows($result);
                                        $formattedCount = str_pad($count, 2, '0', STR_PAD_LEFT);

                                        if ($count === 1) {
                                            echo "<span>" . $formattedCount . "</span> Listing";
                                        } else {
                                            echo "<span>" . $formattedCount . "</span> Listings";
                                        }
                                        ?>
                                    </p>
                                </div>
                            </a>
                        </div>

                        <div class="col-md-4 col-lg-2 category-responsive text-center d-flex align-items-stretch"
                            data-aos="fade-up" data-aos-delay="100" data-aos-duration="1000">
                            <a href="sports.php" class="category-wrap">
                                <div class="category-block">
                                    <div class="category-img">
                                        <svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg"
                                            xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" role="img"
                                            class="iconify iconify--emojione-monotone"
                                            preserveAspectRatio="xMidYMid meet">
                                            <path
                                                d="M32.001 27.927l-3.246 9.989H18.251l8.498 6.174l-3.248 9.988l8.5-6.172l8.498 6.172l-3.248-9.988l8.498-6.174H35.245l-3.244-9.989m2.838 15.12l1.752 5.396l-4.59-3.336l-4.594 3.336l1.756-5.396l-4.592-3.336h5.676l1.754-5.396l1.754 5.396h5.674l-4.59 3.336">
                                            </path>
                                            <path
                                                d="M44.657 26.519v-8.698c0-.364-.199-.67-.481-.86L54 2H35.165l-3.164 4.746L28.837 2H10l9.823 14.96a1.04 1.04 0 0 0-.48.861v8.698C14.862 30.187 12 35.758 12 42c0 11.045 8.955 20 20 20h.001c.681 0 1.354-.035 2.017-.102C44.115 60.887 52 52.365 52 42c0-6.242-2.862-11.813-7.343-15.481M40.826 3h6.328l-8.825 13.239l-3.164-4.746L40.826 3m.667 17.985l.973-1.458c.052.125.082.261.082.404v4.219a.994.994 0 0 1-.297.7C39.251 23.052 35.752 22 32 22a19.87 19.87 0 0 0-10.251 2.851a.996.996 0 0 1-.297-.701v-4.219c0-.143.03-.28.082-.404l.973 1.459h18.986zM16.846 3h6.328l11.324 16.985H28.17L16.846 3M32 59c-9.388 0-17-7.611-17-17s7.612-17 17-17s17 7.611 17 17s-7.612 17-17 17">
                                            </path>
                                        </svg>
                                    </div>
                                    <h6>Sports</h6>
                                    <p class="list">
                                        <?php
                                        $sql = "SELECT * FROM `listings` WHERE `listing_cat` = 'Sports' AND `status` = 'approved'";
                                        $result = mysqli_query($conn, $sql);
                                        $count = mysqli_num_rows($result);
                                        $formattedCount = str_pad($count, 2, '0', STR_PAD_LEFT);

                                        if ($count === 1) {
                                            echo "<span>" . $formattedCount . "</span> Listing";
                                        } else {
                                            echo "<span>" . $formattedCount . "</span> Listings";
                                        }
                                        ?>
                                    </p>
                                </div>
                            </a>
                        </div>

                        <div class="col-md-4 col-lg-2 category-responsive text-center d-flex align-items-stretch"
                            data-aos="fade-up" data-aos-delay="100" data-aos-duration="1000">
                            <a href="health.php" class="category-wrap">
                                <div class="category-block">
                                    <div class="category-img">
                                        <svg viewBox="0 0 32 32"
                                            style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:2;"
                                            version="1.1" xml:space="preserve" xmlns="http://www.w3.org/2000/svg"
                                            xmlns:serif="http://www.serif.com/"
                                            xmlns:xlink="http://www.w3.org/1999/xlink">
                                            <path
                                                d="M4.234,6.266c-1.409,1.45 -2.202,3.415 -2.202,5.464c-0,2.05 0.793,4.015 2.202,5.464l9.625,9.899c0.568,0.584 1.339,0.91 2.143,0.91c0.803,0 1.575,-0.326 2.143,-0.91c1.743,-1.793 9.617,-9.907 9.617,-9.907c1.409,-1.449 2.202,-3.414 2.202,-5.464c0,-2.05 -0.793,-4.014 -2.202,-5.464c-0,0 -0,0 -0,0c-1.419,-1.459 -3.345,-2.277 -5.352,-2.277c-2.007,-0 -3.932,0.818 -5.351,2.277c-0,0 -1.065,1.095 -1.065,1.095c0,0 -1.057,-1.087 -1.057,-1.087c-1.419,-1.459 -3.344,-2.277 -5.351,-2.277c-2.007,0 -3.933,0.818 -5.352,2.277m1.434,1.395c1.039,-1.069 2.448,-1.672 3.918,-1.672c1.47,0 2.878,0.603 3.917,1.672c0,-0 1.774,1.824 1.774,1.824c0.188,0.193 0.447,0.303 0.717,0.303c0.27,-0 0.529,-0.11 0.717,-0.303l1.782,-1.833c1.039,-1.068 2.447,-1.671 3.917,-1.671c1.47,-0 2.878,0.603 3.918,1.671c0.003,0.004 0.006,0.007 0.01,0.011c1.039,1.069 1.626,2.533 1.626,4.059c0,1.526 -0.587,2.991 -1.636,4.07c-0,-0 -7.874,8.114 -9.617,9.907c-0.188,0.193 -0.443,0.304 -0.709,0.304c-0.266,0 -0.521,-0.111 -0.709,-0.304l-9.625,-9.899c-1.049,-1.079 -1.636,-2.544 -1.636,-4.07c-0,-1.526 0.587,-2.99 1.636,-4.069" />
                                            <path
                                                d="M14.998,15.003l-1.997,-0c-0.552,-0 -1,0.448 -1,1c0,0.552 0.448,1 1,1l1.997,-0l-0,1.997c-0,0.552 0.448,1 1,1c0.552,-0 1,-0.448 1,-1l-0,-1.997l1.997,-0c0.552,-0 1,-0.448 1,-1c-0,-0.552 -0.448,-1 -1,-1l-1.997,-0l-0,-1.997c-0,-0.552 -0.448,-1 -1,-1c-0.552,0 -1,0.448 -1,1l-0,1.997Z" />
                                        </svg>
                                    </div>
                                    <h6>Health</h6>
                                    <p class="list">
                                        <?php
                                        $sql = "SELECT * FROM `listings` WHERE `listing_cat` = 'Health' AND `status` = 'approved'";
                                        $result = mysqli_query($conn, $sql);
                                        $count = mysqli_num_rows($result);
                                        $formattedCount = str_pad($count, 2, '0', STR_PAD_LEFT);

                                        if ($count === 1) {
                                            echo "<span>" . $formattedCount . "</span> Listing";
                                        } else {
                                            echo "<span>" . $formattedCount . "</span> Listings";
                                        }
                                        ?>
                                    </p>
                                </div>
                            </a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!-- End Categories Section -->
    <!-- About Section -->
    <section id="about" class="ftco-section ftco-about-section ftco-no-pt ftco-no-pb viewed bg-light">
        <div class="container">
            <div class="row justify-content-center">
                <div class="col-md-5">
                    <div class="styled-heading mb-4 mt-5">
                        <h3><b>ABOUT</b></h3>
                    </div>
                </div>
            </div>
            <div class="row ">
                <div class="col-md-5 d-md-flex align-items-stretch aos-init aos-animate mb-md-5 mb-sm-0"
                    data-aos="fade-up" data-aos-delay="100" data-aos-duration="1000">
                    <div class="img w-100" style="background-image: url(images/about.jpg);"></div>
                </div>
                <div class="col-md-7 mt-0 heading-section aos-init aos-animate" data-aos="fade-up" data-aos-delay="200"
                    data-aos-duration="1000">
                    <div class="pb-md-5">
                        <h2 class="mb-4">DirecTree: Empowering Your Journey Through Every Category</h2>
                        <span class="subheading">Welcome to DirecTree,</span>
                        <p> Your go-to online directory for discovering the best spots and services around town! Whether
                            you're a local or just visiting, DirecTree offers a comprehensive guide across 12 diverse
                            categories to suit every taste and need. Explore <em>Food & Drinks</em> for a range of
                            options from
                            gourmet restaurants to cozy cafes, or check out our <em>Hotels</em> section for
                            accommodations that
                            fit any budget, from luxury stays to charming boutique hotels. The <em>Shopping</em>
                            category will
                            lead you to malls, local boutiques, and specialty shops where you can find everything from
                            fashion to electronics. Indulge in self-care with our <em>Beauty</em> listings, showcasing
                            top-rated
                            salons and spas, or stay active with <em>Fitness</em> options that include gyms, yoga
                            studios, and
                            fitness centers.</p>

                        <p>For those who enjoy nightlife, our <em>Bar & Club</em> section features popular spots for
                            dancing,
                            live music, or a laid-back evening with friends. Looking for fun activities? Check out the
                            <em>Games</em> category for arcades, escape rooms, and more. Discover must-see
                            <em>Places</em> like parks,
                            museums, and cultural landmarks, or enjoy a thrilling experience with our <em>Circus</em>
                            listings
                            featuring unique shows and performances. Movie enthusiasts can find the best <em>Movie
                                Theaters</em>
                            to catch the latest releases, while sports fans can explore the <em>Sports</em> section for
                            local
                            events and facilities. Finally, our <em>Health</em> category provides a directory of
                            clinics,
                            doctors, and wellness centers focused on your well-being.
                        </p>

                        <p>With DirecTree, finding what you need is simple and straightforward. Start exploring now and
                            let us help you uncover the best that your city has to offer!</p>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!-- End About Section -->

    <!-- Blogs Section -->
    <section id="new_blogs" class="ftco-section mb-0">
        <div class="container-xl">
            <div class="row justify-content-center mb-5 mt-5">
                <div class="col-md-7 heading-section text-center" data-aos="fade-up" data-aos-duration="1000">
                    <span class="subheading">Our Blogs</span>
                    <div class="styled-heading mb-3">
                        <h2>Recent Blogs</h2>
                    </div>
                </div>
            </div>
            <div class="row justify-content-center ">

                <?php
                $checkAdmin = "SELECT * FROM `adminlogin`";
                $checkResult = mysqli_query($conn, $checkAdmin);
                $checkRow = mysqli_fetch_assoc($checkResult);
                $admin = $checkRow['username'];

                $sql = "SELECT * FROM `blogs` WHERE `status`='approved' ORDER BY `id` DESC LIMIT 3";
                $result = mysqli_query($conn, $sql);
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_assoc($result)) {
                        $timestamp = $row['add-time'];
                        $date = new DateTime($timestamp);
                        $formattedDate = $date->format('M. d, Y h:i:s A');
                ?>
                        <div class="col-md-3 d-flex mb-4">
                            <a href="blog.php #blogs">
                                <div class="blog-entry justify-content-center" data-aos="fade-up" data-aos-duration="1000"
                                    data-aos-delay="100">
                                    <div class="text">
                                        <h3 class="heading mb-2"><?php
                                                                    echo substr($row['title'], 0, 30) . '...';
                                                                    ?></h3>
                                        <p class="mb-2">
                                            <?php
                                            echo substr($row['content'], 0, 100) . '...';
                                            ?>
                                        </p>
                                        <div class="list-team mb-1">
                                            <h3 class="mb-0"><?php
                                                                if ($row['username'] === $admin) {
                                                                    echo strtoupper($admin) . ' | Admin';
                                                                } else {
                                                                    echo strtoupper($row['username']);
                                                                }
                                                                ?> </h3>
                                        </div>
                                        <p class="meta mb-0"><span><?php
                                                                    echo $formattedDate;
                                                                    ?></span></p>

                                    </div>
                                </div>
                            </a>
                        </div>
                <?php
                    }
                }
                ?>
                <div class="container text-center mb-0">
                    <a href="blog.php">
                        <button class="btn btn-outline-light">
                            View All</button>
                    </a>
                </div>
            </div>
        </div>
    </section>
    <!-- End Blogs Section -->
    <?php
    include 'partials/_footer.php';
    ?>
    <div id="preloader">
        <div class="spinner"></div>
    </div>
    <!-- jQuery, Bootstrap JS. -->
    <!-- jQuery first, then Popper.js, then Bootstrap JS -->
    <script src="js/preloader.js"></script>
    <script src="js/jquery-3.2.1.min.js"></script>
    <script src="js/popper.min.js"></script>
    <script src="js/bootstrap.min.js"></script>
    <!-- Optional JavaScript; choose one of the two! -->

    <!-- Option 1: Bootstrap Bundle with Popper -->
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.bundle.min.js"
        integrity="sha384-MrcW6ZMFYlzcLA8Nl+NtUVF0sA7MsXsP1UyJoMp4YLEuNSfAP+JcXn/tWtIaxVXM"
        crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/aos@2.3.4/dist/aos.js"></script>
    <script>
        AOS.init({
            once: true
        });
    </script>

    <!-- Option 2: Separate Popper and Bootstrap JS -->
    <!--
    <script src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.9.2/dist/umd/popper.min.js" integrity="sha384-IQsoLXl5PILFhosVNubq5LC7Qb9DXgDA9i+tQ8Zj3iwWAwPtgFTxbJ8NT4GN1R8p" crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.min.js" integrity="sha384-cVKIPhGWiC2Al4u+LWgxfKTRIcfu0JTxR+EQDz/bgldoEyl4H0zUF0QKbrJ0EcQF" crossorigin="anonymous"></script>
    -->
</body>

</html>